...
!function ($) {
  ...
  * ============================== */
  ...
  var Filters = function (filter, options) {
    this.$filter = filter
    this.$parent = this.$filter.parent()
    this.$filters = this.$parent.children()
    this.$listItems = $(this.$parent.data('filter')).children()
    this.options = $.extend({}, $.fn.filters.defaults, options)
  }

  Filters.prototype.filter = function () {
    var filterValue = this.$filter.data('value')

    if (this.$filter.hasClass('filter-active')) {
      this.$filter.removeClass('filter-active')
      var $filtered1 = this.$listItems.filter(function() {
        return $(this).data('value') != filterValue
      }).show()
      this.filipewl2($filtered1)
    } else {
      this.$filter.addClass('filter-active')
      var $filtered = this.$listItems.filter(function() {
        return $(this).data('value') != filterValue
      }).hide()
      this.filipewl($filtered)
    }
  }

  Filters.prototype.filipewl = function($elements) {
  ...
  Filters.prototype.filipewl2 = function($elements) {
    return $elements.each(function() {
      var html = $(this).html()
        , data = $(this).data()
        , attributes = $(this).prop('attributes')
  ...
  $.fn.filters = function (option) {
    return this.each(function () {
      var $filter = $(this)
        ...
        , options = typeof option == 'object' && option
      if (!data) $filters.data('filters', (data = new Filters($filter, options)))
      else $filters.data('filters').$filter = $filter
